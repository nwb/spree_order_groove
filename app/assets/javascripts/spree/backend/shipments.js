...
$(document).ready(function () {
  ...
  $('a.save-item').click(function(){
    ...
    var subscribe = save.parents('tr').find("#item_auto_delivery_true:checked").val();
      //var delivery_number = 1000; //$("input.delivery_number[data-variant-id='" + variant_id + "']").val();
    var frequency = save.parents('tr').find("select#item_frequency").val();

    toggleItemEdit();
    adjustShipmentItems(shipment_number, variant_id, quantity, subscribe, frequency);
    return false;
  });
...
adjustShipmentItems = function(shipment_number, variant_id, quantity, auto_delivery, frequency){
    var shipment = _.findWhere(shipments, {number: shipment_number + ''});
