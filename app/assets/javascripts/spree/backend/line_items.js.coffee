$(document).ready ->
  ...
  $('a.save-line-item').click ->
    ...
    auto_delivery = save.parents('tr').find('input[name="item[auto_delivery]"]:checked').val()
    frequency = parseInt(save.parents('tr').find('#item_frequency').val())

    toggleItemEdit()
    options={auto_delivery:auto_delivery, frequency: frequency}
    adjustLineItem(line_item_id, quantity, options)
