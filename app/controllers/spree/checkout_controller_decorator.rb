  ...
  def create_autodelivery_order
      if @order.completed?
        ...
        if Spree::Promotion::Rules::Autodelivery.new.eligible?(@order) && !!@order.user  # if this order is ready to signup
          ...
          payment={:cc_holder=>Base64.encode64(rc4.encrypt(billing_address.firstname + ' ' + billing_address.lastname)).chomp, :cc_type=>(@order.payments.first.source.cc_type || 'visa'),:cc_number=> session[:cc].chomp(),:cc_exp_date=>Base64.encode64(rc4.encrypt(((@order.payments[0].source[:month].to_i<10 ? '0' : '') +@order.payments[0].source[:month] + '/' + @order.payments[0].source[:year]))).chomp }
