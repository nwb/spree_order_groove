...
namespace :spree do
  namespace :extensions do
    namespace :ordergroove do
      ...
      task :upload_feeds => :environment do |t|
          start_time = Time.now
          report_str = [ "Uploading og feeds: #{start_time.strftime( "%Y-%m-%d %H:%M:%S" )}"]
          success = true
          report_str << "Processing..."
          complete, report = upload_feeds( start_time )
          report_str << report
          end_time = Time.now
          report_str << "Finished uploading og feeds:  #{start_time.strftime( "%Y-%m-%d %H:%M:%S" )}"
          report_str << "Elapsed time: #{(end_time - start_time) rescue 'unknown'} seconds"
          puts report_str
      end

      def upload_feeds  timestamp
        config=Spree::Config
        report = ""
        #sftp
        ...
        Net::SFTP.start(config["og_#{site}_ftp_host_#{ENV["RAILS_ENV"]}"], config["og_#{site}_ftp_user"], :password => config["og_#{site}_ftp_pass"]) do |sftp|
           ...
           report << "get the #{site} feeds"
           file_content=get_html_content("http://#{ ENV["RAILS_ENV"]=='staging' ? 'staging' : 'www' }.#{ 'nwb'==site ? 'natural' : 'pet' }wellbeing.com/feed/ogproducts.csv")
                      filename=config["#{site}_og_merchant_id"] + ".Products.csv"
                      sftp.file.open(filename, "w") do |f|
                          report << "upload the #{site} feeds"
                          f.puts file_content
                      end
        end
        end
