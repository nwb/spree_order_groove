...
namespace :spree do
  namespace :extensions do
    namespace :ordergroove do
      desc "upload og categories"

      task :upload_categories => :environment do |t|
        start_time = Time.now
        report_str = [ "Uploading og categories: #{start_time.strftime( "%Y-%m-%d %H:%M:%S" )}"]
        success = true
        report_str << "Processing..."
        complete, report = upload_categories( start_time )
        report_str << report
        end_time = Time.now
        report_str << "Finished uploading og categories:  #{start_time.strftime( "%Y-%m-%d %H:%M:%S" )}"
        report_str << "Elapsed time: #{(end_time - start_time) rescue 'unknown'} seconds"
        puts report_str
      end

      def upload_categories  timestamp
        report = ""
        #sftp
        Spree::Store.all.each do |store|
          ...
          Net::SFTP.start(config["og_ftp_host_#{ENV["RAILS_ENV"]}"], config["og_ftp_user"], :password => config["og_ftp_pass"]) do |sftp|
            ...
            report << "get the #{store.code} categories"
            file_content=get_html_content("https://#{store.url}/feed/ogcategories.xml")
            filename=config["og_merchant_id"] + ".ProductCategories.xml"
            sftp.file.open(filename, "w") do |f|
              report << "upload the #{store.code} categories"
              f.puts file_content
            end
          end
        end
